...
public class ContatoRepository {
	...
	public List<TipoQuantidadeDto> countByTipo(Integer idUsuario) throws Exception {
		...
		String query = "select " + "case " + "when tipo = 1 then 'FAMILIA' " + "when tipo = 2 then 'AMIGOS' "
				+ "when tipo = 3 then 'TRABALHO' " + "when tipo = 4 then 'OUTROS' " + "end " + "as tipo, "
				+ "count(*) as quantidade " + "from contato " + "where idusuario = ? " + "group by tipo";
